...
// D016: torch headers pull <intrin.h> under -D_MSC_VER, clashing with clang's
// own intrinsics on the standalone hipcc -c host pass. For the device-only
// compile (VLLM_DEVICE_ONLY) skip them; the host TU is built separately by MSVC.
...
// Arch-family markers expected by the dispatch ladder at the bottom of this
// file (#if defined(__HIP__GFX9__)/#elif __GFX11__/#elif __GFX12__). Upstream
// vLLM injects these via the CMake arch list; the Windows standalone build
// does not, so derive them from the __gfxNNN__ defines clang passes for
// --offload-arch=gfxN.
  ...
  #define __GFX12__
#endif
// Whether the toolchain exposes the gfx12 WMMA intrinsics. The ROCm 7.16
// preview on gfx1201 (RX 9070 XT) does NOT lower
// __builtin_amdgcn_wmma_f32_16x16x16_f16_w32_gfx12, so the scalar-fma fallback
// (see gcn_wmma16x16x16_instr in the __GFX12__ branch) is selected instead.
// Define __GFX12_HAS_WMMA__ when targeting a ROCm whose backend ships it.
#if defined(__HIPCC__) && defined(__gfx942__)
  // MI300x-class already ships the intrinsic; gfx942 is our proxy for
  // "backend has gfx12 WMMA lowering".
...
__device__ __forceinline__ floatx8 gcn_wmma16x16x16_instr(const bit16x8& inpA,
  ...
  return inpC;
   #else
  // ROCm 7.16+ (shipped for RDNA4) exposes the gfx12 WMMA intrinsics; on
  // earlier toolchains (e.g. the 7.16 preview on RX 9070 XT) the
  // `_w32_gfx12` builtins are not lowered for gfx1201, so fall back to a
  // real scalar fp16/bf16 -> fp32 16x16x16 matmul. D016: math-equivalent,
  // same D = A.B + C semantics, just register-tiled fmaf.
