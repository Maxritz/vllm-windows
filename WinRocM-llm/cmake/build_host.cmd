...
set CL=%CLANG% -c -x c++ -std=c++20 -m64 -DNDEBUG -I%SRC% -Icsrc -Isrc\engine\include -I"%PROJECT_ROOT%" -DUSE_ROCM -DC10_CUDA_NO_CMAKE_CONFIGURE_FILE -DC10_CUDA_BUILD_SHARED_LIBS -D__HIP_PLATFORM_AMD__=1 -fms-extensions -fms-compatibility -isystem "csrc/hip_wrap" -isystem "%TORCH_INC%" -isystem "%TORCH_INC%\torch\csrc\api\include" -isystem "%ROCM_HOME%\include" -isystem "%PY_INC%" -D__NO_MATH_DEFINES=1
